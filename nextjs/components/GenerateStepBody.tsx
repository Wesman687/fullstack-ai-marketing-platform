...
import { GeneratedContent } from "@/server/db/schema/schema";
    ...
    const handleSave = async (id: string) => {
        ...
        try {
            await axios.patch(`/api/projects/${projectId}/generated-content/`, {
                id,
                result: editingContent,
            })
            setGeneratedContent((prev) =>
                prev.map((content) =>
                    content.id === id ? { ...content, result: editingContent } : content
                )
            );
    ...
    <div>
      ...
      <div>
        {generatedContent.map((content) => (
          ...
          >
            <CardContent className="p-5 lg:p-8 spay2 flex-grow min-w-0">
