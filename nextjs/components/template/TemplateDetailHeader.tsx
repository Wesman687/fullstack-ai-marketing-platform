...
interface TemplateDetailHeaderProps {
    template: Template
    setShowTemplateDeleteConfirmation: (show: boolean) => void
}
function TemplateDetailHeader({ template, setShowTemplateDeleteConfirmation }: TemplateDetailHeaderProps) {
    const [title, setTitle] = useState(template.title);
    const [isEditing, setIsEditing] = useState(false);

    const handleTitleSubmit = async () => {
        try {
            const response = await axios.patch(`/api/template/${template.id}`, {
                title,
            });
